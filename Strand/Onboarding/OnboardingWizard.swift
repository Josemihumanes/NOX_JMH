...
//
// A full-screen, paged onboarding + pairing flow for NOX. Cinematic and calm:
...
//
// Steps:
//  1 Welcome           — NOX + "all your data, none of the cloud"
...
public struct OnboardingWizard: View {
    ...
    private var ctaTitle: String {
        ...
        case .done:       return String(localized: "Enter NOX")
        }
    }
...
private struct WhatItDoesStep: View {
    ...
    var body: some View {
        StepShell(title: String(localized: "What NOX does"), subtitle: String(localized: "Three quiet promises.")) {
            VStack(spacing: 14) {
        ...
        StepShell(title: String(localized: "What to expect"),
            ...
            VStack(spacing: 12) {
                ...
                expectationRow(
                    ...
                    body: String(localized: "On iPhone this is a sideloaded build. Re-sign it about every 7 days on a free Apple ID (longer on a paid account). After your phone reboots, unlock it once so NOX can read and sync its data.")
                )
                .opacity(shown ? 1 : 0)
                .offset(y: shown ? 0 : 8)
        ...
        StepShell(title: String(localized: "A quick word before we connect"),
            ...
            VStack(spacing: 24) {
                ...
                InfoCard(
                    ...
                    message: String(localized: "NOX talks to your strap directly over Bluetooth Low Energy. There's no server in the middle. The connection is local, and so is every reading it pulls in.")
                )

                Text("When the system prompt appears, choose Allow so NOX can find your strap.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .multilineTextAlignment(.center)
                    .frame(maxWidth: 460)
...
private struct ScanStep: View {
    ...
    var body: some View {
        StepShell(title: String(localized: "Find your strap"),
                  subtitle: live.bonded ? String(localized: "Bonded. You're set.") : String(localized: "Pick your strap below, then tap Scan. NOX will find it.")) {
            VStack(spacing: 24) {
                ...
                if !live.bonded {
                    ...
                    // WHOOP is NOX's primary band, so onboarding leads with it — but it isn't required.
    ...
    private var reassurance: some View {
        StrandCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("WHOOP straps don't appear in your \(Platform.deviceNoun)'s Bluetooth settings. They advertise on a custom profile that only apps like NOX can find, so there's nothing to pair there, and you shouldn't try.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
...
private struct NotificationsStep: View {
    ...
    var body: some View {
        StepShell(title: String(localized: "Stay in the loop"),
                  subtitle: String(localized: "NOX can tap your wrist when your \(Platform.deviceNoun) needs you. No glance at the screen required.")) {
            VStack(spacing: 24) {
                ZStack {
                    Circle()
                        .stroke(StrandPalette.accent.opacity(0.25), lineWidth: 2)
                ...
                // keep only what iOS genuinely does: NOX's own strain nudges + smart alarm buzz the strap
                // directly over BLE.
                InfoCard(
                    icon: "applewatch.radiowaves.left.and.right",
                    tint: StrandPalette.statusPositive,
                    title: String(localized: "A buzz, not a banner"),
                    message: String(localized: "NOX taps your strap so an alert lands on your wrist instead of your screen. No need to reach for it. Everything stays on \(Platform.deviceNounPhrase).")
                )
                ...
                InfoCard(
                    ...
                    tint: StrandPalette.statusPositive,
                    title: String(localized: "A buzz, not a banner"),
                    message: String(localized: "When the \(Platform.deviceNoun) apps you choose send a notification, NOX taps your strap: Slack, Calendar, Messages, whatever matters. Everything stays on \(Platform.deviceNounPhrase).")
                )
            ...
            VStack(spacing: 22) {
                ...
                VStack(spacing: 10) {
                    Text("Your thread starts here.")
                        .font(StrandFont.title1)
                        .foregroundStyle(StrandPalette.textPrimary)
                    Text("Every beat, every night, every day, woven into one quiet picture of you. Welcome to NOX.")
                        .font(StrandFont.body)
                        .foregroundStyle(StrandPalette.textSecondary)
                        .multilineTextAlignment(.center)
                        .frame(maxWidth: 420)
...
private struct AppearanceStep: View {
    ...
    var body: some View {
        StepShell(title: String(localized: "Make it yours"),
                  subtitle: String(localized: "Choose how NOX looks. The whole app updates as you tap. You can change this any time in Settings → Appearance.")) {
            VStack(spacing: 28) {
