...
/// 4-CSV zip via StrandImport.WhoopCsvExporter. The zip re-imports into NOX on Mac (Data Sources →
    ...
    static func run(repo: Repository) async -> ExportResult {
        ...
        do {
            ...
            let tmp = try await Task.detached(priority: .utility) {
                ...
                // omitted (read only the two NOX sources), matching the cycles/sleep exclusion.
            ...
            panel.title = String(localized: "Export NOX data as CSV")
