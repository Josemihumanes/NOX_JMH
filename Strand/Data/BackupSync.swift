...
/// get automatic off-device backup with no in-app cloud account - NOX only writes a local file; the
...
enum FolderBackup {
    ...
    static func pickFolder() -> URL? {
        ...
        panel.message = String(localized: "Choose a folder for NOX backups (for example a Google Drive or iCloud folder).")
