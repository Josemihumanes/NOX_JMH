...
///
/// NOX keeps everything in one SQLite file (`<AppSupport>/OpenWhoop/whoop.sqlite`, plus the
...
enum DataBackup {
    ...
    static func runExport(checkpoint: @escaping () async -> Bool) async -> BackupResult {
        ...
        catch { return .failure(String(localized: "Couldn't locate the NOX database. \(error.localizedDescription)")) }

        let dbURL = URL(fileURLWithPath: dbPath)
        guard FileManager.default.fileExists(atPath: dbPath) else {
            return .failure(String(localized: "There's no NOX data to export yet. Import or record some first."))
        }
        ...
        panel.title = String(localized: "Export NOX backup")
    ...
    private struct ExportIntegrityFailure: LocalizedError {
        ...
        var errorDescription: String? {
            String(localized: "the NOX database failed its integrity check (SQLite reports: \(complaint)). A backup of it would not restore. Export the WHOOP-format CSV (Settings → Export data) to save what's still readable.")
        }
    }
    ...
    static func writeBackup(checkpoint: @escaping () async -> Bool, to dest: URL) async -> BackupResult {
        ...
        let dbURL = URL(fileURLWithPath: dbPath)
        guard FileManager.default.fileExists(atPath: dbPath) else {
            return .failure(String(localized: "There's no NOX data to export yet."))
        }
    ...
    static func runImport() async -> BackupResult {
        ...
        panel.title = String(localized: "Import NOX backup")
    ...
    static func restore(from pickedSource: URL, toDatabaseAt dbPath: String,
        ...
        guard isSQLiteFile(at: source) else {
            return .failure(String(localized: "That file isn't a NOX backup. It doesn't look like a SQLite database."))
        }
        ...
        // NOX-Mac/iOS backup always carries `grdb_migrations`; reject everything else that holds data.
        ...
        if origin == .android || (origin == .unknown && holdsData) {
            return .failure(String(localized: "This isn't a NOX backup from this app. It's missing the migration bookkeeping a NOX backup carries (it looks like an Android backup or another app's database), and restoring it would strand your store. To move your history across platforms, export the WHOOP-format CSV on the other device (Settings → Export data) and import that here, or import your original WHOOP / Apple Health export."))
        }
    ...
    /// "NOX-backup-2026-06-07.noopbak"
    private static func defaultBackupName() -> String {
        let f = DateFormatter()
        f.locale = Locale(identifier: "en_US_POSIX")
        f.dateFormat = "yyyy-MM-dd"
        return "NOX-backup-\(f.string(from: Date())).noopbak"
    }
    ...
    /// Which platform produced a NOX backup, judged by its migrator's bookkeeping table.
