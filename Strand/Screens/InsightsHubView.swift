    ...
    private var moversSection: some View {
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            ...
            if model.ranked.isEmpty {
                NoopCard {
                    Text(String(localized: "Not enough overlap between your journal answers and \(outcome.outcomeName.lowercased()) yet. Keep logging. Each behaviour needs days both with and without it before NOX can read its effect."))
    ...
    private var doseSection: some View {
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            ...
            if model.doseCards.isEmpty {
                NoopCard {
                    Text(String(localized: "Log alcohol or late caffeine with an amount and NOX fits a personal dose curve: how much each extra unit tends to move your numbers. Until then it shows typical patterns, clearly labelled as not yet yours."))
                        .font(StrandFont.subhead)
                        .foregroundStyle(StrandPalette.textSecondary)
                        .fixedSize(horizontal: false, vertical: true)
                        .frame(maxWidth: .infinity, alignment: .leading)
