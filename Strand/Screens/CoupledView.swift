...
// glance back. NOX's Today stays the default and is untouched.
//
...
struct CoupledView: View {
    ...
    /// Effort strain on NOX's 0–100 axis for the day (stored row; no live recompute here, this is a
    ...
    // (a coupled read of NOX's OWN scores, same data, different lens) without the branding word. The
    // matching Android caption is byte-identical.
    private var footerCaption: some View {
        Text("A classic one-glance read of NOX's own scores. Same data, different lens.")
            .font(StrandFont.footnote)
            .foregroundStyle(StrandPalette.textTertiary)
