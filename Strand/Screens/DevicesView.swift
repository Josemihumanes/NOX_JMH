...
// MARK: - Devices
//
// Pair and manage the bands NOX reads from. WHOOP-FIRST: the WHOOP is the primary, fully-supported
...
struct DevicesView: View {
    ...
    var body: some View {
        ScreenScaffold(title: "Devices",
                       subtitle: "Pair and manage the bands NOX reads from.",
            ...
            if let registry = model.deviceRegistry {
                ...
                DataPendingNote(
                    ...
                    message: "NOX is opening your on-device data. Your paired bands will appear here in a moment.",
...
private struct DevicesContent: View {
    ...
    var body: some View {
        ...
        .alert("Remove this device?",
            ...
            Text("Remove \(device.displayName)? NOX will stop connecting to it. Its recorded data is kept and you can re-add it any time.")
        }
    ...
    private var whoopFirstFooter: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            Text("WHOOP is NOX's primary, fully-supported band. Other heart-rate straps are an early, in-development addition: they stream live heart rate and HRV, but not WHOOP's deeper sleep and recovery data.")
    ...
    private func confirmRemove(_ device: PairedDevice) {
        ...
        // #78: actually RELEASE the BLE link, not just archive the registry row — otherwise NOX keeps
    ...
    private var cardContent: some View {
        StrandCard(padding: 18, tint: isActive ? StrandPalette.accent : nil) {
            VStack(alignment: .leading, spacing: NoopMetrics.cardInnerSpacing) {
                ...
                // was reset again or re-claimed in the Oura app, NOX no longer owns it) without faking a
                ...
                // What NOX USES it for — the scores/screens this device drives.
                ...
                // banks no history in that state, which otherwise looks like a NOX sync bug).
    ...
    private var ouraLocalStateNote: some View {
        HStack(alignment: .top, spacing: 6) {
            Image(systemName: "info.circle")
                ...
                .frame(width: 14)
                .accessibilityHidden(true)
            Text("Paired locally. NOX owns this ring while it holds the key. If you reset it again or set it up in the Oura app, NOX no longer owns it and you would re-add it to take it over.")
                .font(StrandFont.caption)
                .foregroundStyle(StrandPalette.statusWarning)
                .fixedSize(horizontal: false, vertical: true)
        }
...
/// Honest, per-model summary of what a device captures and what NOX uses it for — shown on its card.
///
...
/// and Charge/Effort/Rest are NOX-derived scores. Verdicts are source-verified against the decode +
...
struct DeviceCapabilityProfile {
    ...
    let powers: String         // the NOX scores / screens this device drives
    ...
    static func make(for d: PairedDevice) -> DeviceCapabilityProfile {
        ...
        if d.sourceKind == .huami {
            return DeviceCapabilityProfile(
                ...
                footnote: String(localized: "Experimental: live heart rate where the band exposes it. Some bands need a pairing we can't do yet. NOX will say so honestly and never show a made-up number. No sleep, recovery, skin temp, SpO₂ or steps."))
        }
        // EXPERIMENTAL locally-adopted Oura ring (gen 3/4/5). The gen is carried on `model` ("Oura Ring
        // 3/4/5") and recovered with OuraRingGen.from(model:). NOX reads the ring's OWN raw signals + open
        ...
        // WHOOP 4.0 — NOX's primary band; no steps over BLE.
        if model.contains("4") {
            return DeviceCapabilityProfile(
...
struct DeviceCardCatalog: View {
    ...
    var body: some View {
        ScreenScaffold(title: "Devices",
                       subtitle: "What each band captures (and what NOX uses it for).",
                       topBackground: liquidScaffoldSky()) {
