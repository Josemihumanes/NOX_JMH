...
struct DataSourcesView: View {
    ...
    // own source, refresh. The brand's own scores are stored as reference only, never NOX scores.
    ...
    // "Broadcast heart rate" (opt-in, OFF by default): make NOX a standard BLE Heart Rate peripheral
    // (0x180D / 0x2A37) so a gym treadmill / Zwift / Peloton can read the live strap HR NOX receives.
    ...
    /// score is NEVER mapped to a NOX Charge/Effort/Rest — NOX recomputes its own from the raw inputs.
    ...
    private var broadcastHrCard: some View {
        ...
        return card(title: String(localized: "Broadcast heart rate"), icon: "dot.radiowaves.up.forward",
            ...
            Text("Acts as a standard Bluetooth heart-rate strap. Pair NOX from your treadmill, bike or app to see your strap's heart rate there.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)

            // FI-2 (#490) — the 4.0-vs-5.0 explainer. Broadcast works for BOTH strap generations because it
            // re-shares whatever LIVE heart rate NOX already has off the strap; it doesn't depend on the
    ...
    private var generationExplainer: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            generationRow(title: "WHOOP 5.0 & MG",
                          detail: String(localized: "Broadcasts the live heart rate NOX derives from the strap once connected."))
        }
        .padding(.top, 2)
