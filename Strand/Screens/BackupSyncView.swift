...
struct BackupSyncView: View {
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center, spacing: 16) {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("Backs up to your folder about once a day and keeps the latest \(FolderBackup.keepCount). On this platform it runs when you next open NOX.")
                            .font(StrandFont.footnote).foregroundStyle(StrandPalette.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                    Spacer(minLength: 0)
    ...
    private func chooseFolder() {
        ...
        // the user with only Cancel and NOX silently doing nothing. We can't tell a deliberate Cancel
        ...
        Task {
            if await FolderBackup.pickFolder() != nil {
                ...
                alertMessage = String(localized: "NOX didn't get a folder back from the picker. If the Select button won't enable, try creating a fresh folder in Files (under On My iPhone or iCloud Drive) and choosing that instead.")
                showAlert = true
            }
        }
        #endif
    ...
    private func openRestorePicker() {
        ...
        if snapshots.isEmpty {
            ...
            alertMessage = String(localized: "There are no NOX backups in your folder yet. Use Back up now first.")
            showAlert = true
        } else {
        ...
        Task {
            ...
            await MainActor.run {
                ...
                case .imported:
                    ...
                    alertMessage = String(localized: "Fully quit and reopen NOX to load it.")
                case .failure(let m):
