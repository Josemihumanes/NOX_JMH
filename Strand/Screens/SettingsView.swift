...
struct SettingsView: View {
    ...
    // it's shown on NOX's 0–100 axis or WHOOP's 0–21 Day Strain axis.
    ...
    /// "How NOX works" primer sheet (the four-section explainability primer), reachable any
    ...
    var body: some View {
        ScreenScaffold(title: "Settings",
                       subtitle: "Your numbers, your strap, and how NOX works. All on \(Platform.deviceNounPhrase).",
    ...
    /// iOS 16+ and macOS 13+ (NOX's floor), so the same control serves both platforms — no
    /// availability gating needed. The photo is stored only on this device (NOX is fully offline).
    private var profilePhotoCard: some View {
        SettingsSection(
            icon: "person.crop.circle",
            title: "Profile photo",
            blurb: "Optional. Add a photo for the avatar in the top-left. Stored only on \(Platform.deviceNounPhrase). NOX is offline, so it's never uploaded."
        ) {
            HStack(spacing: 16) {
    ...
    private var profileCard: some View {
        SettingsSection(
            ...
            VStack(spacing: 0) {
                ...
                Text("Counter ticks per step. Leave at 1.0 unless your steps run high. On a WHOOP 5/MG they can run very high (10× or more), so this goes up to 30. Walk a known 1,000 steps and divide NOX's count by the real count to get your value.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
                rowDivider
                // Tap-through to the WHOOP 4.0 steps-ESTIMATE calibration (a SEPARATE thing from the
                // 5/MG @57 counter divisor above): a 4.0 sends no step count, so NOX estimates steps
                ...
                Text("For a WHOOP 4.0, which sends no step count: NOX estimates steps from motion, calibrated to your phone. Tap to see how close it is and adjust it.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    /// changes, NOX keeps everything in SI and converts at the point of display.
    private var unitsCard: some View {
        SettingsSection(
            ...
            VStack(spacing: 0) {
                ...
                rowDivider
                // Effort scale (#268) — show NOX's native 0–100 Effort or WHOOP's 0–21 Day Strain axis.
        ...
        SettingsSection(
            ...
            VStack(spacing: 0) {
                ...
                FormRow(label: "Chart colours") {
                    // Default = NOX's clean metric ramps; Classic = the throwback red→amber→green
    ...
    private var strapCard: some View {
        SettingsSection(
            ...
            title: "Strap",
            blurb: "NOX pairs directly with your WHOOP over Bluetooth: no WHOOP app, no cloud."
        ) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                Text("Keeps the detailed beat-to-beat heart-rate stream running all day and night, not just while a live screen is open, so NOX captures much more for overnight HRV, recovery and sleep. Uses more battery: your strap streams heart rate continuously while connected.")
                    .font(StrandFont.caption)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private func recalibrateHrvBaseline() {
        ...
        backupAlertMessage = String(localized: "NOX will re-learn your baseline from tonight's data onward. Your history is kept, and it takes a few nights to settle.")
        showBackupAlert = true
    }
    ...
    private var sleepStagingCard: some View {
        SettingsSection(
            ...
            blurb: "How NOX splits a night into light / deep / REM. This is a separate, opt-in recipe. Your default staging is unchanged unless you turn it on."
        ) {
            VStack(alignment: .leading, spacing: NoopMetrics.rowSpacing) {
    ...
    private var deepDataButtonReason: String {
        ...
        if !live.encryptedBond {
            return String(localized: "Needs the full encrypted bond: close the official WHOOP app and pair the strap to NOX first (a live-HR-only link can't carry the unlock).")
        }
        return live.worn
    ...
    private var fiveMGCard: some View {
        SettingsSection(
            ...
            VStack(alignment: .leading, spacing: NoopMetrics.rowSpacing) {
                ...
                .toggleStyle(.switch)
                .tint(StrandPalette.accent)
                Text("On a 5/MG connection NOX will send a puffin realtime-stream request after the handshake, and log what comes back. If you have a 5/MG strap, turning this on and sharing your strap log helps map the protocol. No effect on WHOOP 4.0.")
                    .font(StrandFont.caption)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    /// Raw-sensor CSV export — a read-only diagnostic over the decoded streams NOX already stores
    ...
    private var rawSensorDiagnosticsCard: some View {
        SettingsSection(
            ...
            title: "Diagnostics",
            blurb: "A read-only export of the decoded sensor streams NOX already stores. Works on any strap. Nothing is written to your device, and nothing is uploaded."
        ) {
            VStack(alignment: .leading, spacing: NoopMetrics.rowSpacing) {
                // MARK: Export raw sensor data (CSV) — a read-only diagnostic over the decoded streams
                // NOX already stores (HR, R-R, motion, steps, PPG-HR, SpO₂, skin temp, resp, events).
                Button {
                    exportRawSensorCSV()
                } label: {
                    if rawCsvBusy {
    ...
    /// NOX near the chosen time, never guaranteed to the minute.
    ...
    private var debugExportCaption: String {
        #if os(iOS)
        return String(localized: "Writes a timestamped copy of your strap log to NOX's folder in the Files app, once a day. Handy for a bug report without remembering to grab it. On iPhone it fires when iOS next wakes NOX near your chosen time, not guaranteed to the minute (keep NOX open overnight for the best chance). Everything stays on \(Platform.deviceNounPhrase); nothing is uploaded.")
        #else
        return String(localized: "Writes a timestamped copy of your strap log to your Documents folder, once a day. Handy for a bug report without remembering to grab it. On Mac it runs while NOX is open (and catches up on launch if the time passed while it was closed). Everything stays on \(Platform.deviceNounPhrase); nothing is uploaded.")
        #endif
    }
    ...
    private func runScheduledExportNow() {
        ...
        if let url {
            ...
            #if os(iOS)
            backupAlertMessage = String(localized: "Saved \(url.lastPathComponent) to NOX's folder in the Files app.")
            #else
    ...
    private var backupCard: some View {
        SettingsSection(
            ...
            blurb: "Move all your NOX data to another machine. Export saves everything (history, sleeps, workouts, settings) to a single file you can copy across; import replaces \(Platform.deviceNounPhrase)'s data with a backup."
        ) {
    ...
    private func runCsvExport() {
        ...
        Task {
            ...
            case .exported(let url):
                ...
                backupAlertMessage = String(localized: "Saved to \(url.lastPathComponent). The zip re-imports into NOX (Data Sources → WHOOP Export) on any Mac, iPhone, or Android device.")
                showBackupAlert = true
    ...
    private func handleBackup(_ result: DataBackup.BackupResult) {
        ...
        case .imported:
            ...
            backupAlertMessage = String(localized: "Your data has been restored. Quit and reopen NOX for it to take effect.")
            showBackupAlert = true
        case .failure(let message):
    ...
    private var aboutCard: some View {
        SettingsSection(
            ...
            title: "About",
            blurb: "NOX: all your data, none of the cloud."
        ) {
            VStack(alignment: .leading, spacing: 16) {
                HStack(spacing: 10) {
                    Text("NOX")
                ...
                // How NOX works — the plain-English primer: how sleep is sorted, how scores +
                ...
                .buttonStyle(LiquidPressStyle())
                .accessibilityLabel("How NOX works")
                ...
                // About Apple Watch data: the honest capability/confidence page for running NOX off
                    ...
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            Text("About Apple Watch data")
                                .font(StrandFont.body)
                                .foregroundStyle(StrandPalette.textPrimary)
                            Text("Use NOX with just an Apple Watch. What it's great at, and where it's lighter than a strap.")
                                .font(StrandFont.footnote)
                                .foregroundStyle(StrandPalette.textTertiary)
                                .fixedSize(horizontal: false, vertical: true)
                        }
                    ...
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            Text("Storage")
                                .font(StrandFont.body)
                                .foregroundStyle(StrandPalette.textPrimary)
                            Text("Where NOX's on-device space is going, and a one-tap clean-up.")
                                .font(StrandFont.footnote)
                                .foregroundStyle(StrandPalette.textTertiary)
                                .fixedSize(horizontal: false, vertical: true)
                        }
                ...
                // Project home — NOX's code, releases, issues and wiki live on GitHub
                ...
                Text("A standalone companion for your WHOOP. Everything stays on this device: your history, your live stream, your numbers. Nothing is uploaded. NOX is an independent, experimental project, not the WHOOP app.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
                ...
                HStack(alignment: .top, spacing: 10) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        ...
                        .font(.system(size: 13))
                        .accessibilityHidden(true)
                    Text("NOX is not a medical device. It is for informational and personal-insight purposes only and is not intended to diagnose, treat, cure or prevent any condition. Talk to a clinician for medical advice.")
                        .font(StrandFont.footnote)
                        .foregroundStyle(StrandPalette.textSecondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    /// Calm, honest "what to expect running NOX on iPhone" callout — sideloading reality, re-sign
    ...
    private var iphoneExpectations: some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            iphoneExpectationLine(String(localized: "After your iPhone reboots, unlock it once. Until you do, iOS keeps NOX's files locked (Data Protection), so new history can't be written or synced."))
            iphoneExpectationLine(String(localized: "Background Bluetooth has OS limits: iOS may pause NOX when it's not in the foreground, so keep it open while syncing a fresh strap."))
...
/// A custom header (not SwiftUI's `DisclosureGroup`) is used so it matches NOX's near-black
...
/// then content. A faint accent-blue wash anchors the card to NOX's neutral chrome (WHOOP skin).
        ...
        NoopCard {
            VStack(alignment: .leading, spacing: 10) {
                Label("How this works", systemImage: "figure.walk.motion")
                    ...
                    .foregroundStyle(StrandPalette.textPrimary)
                Text("NOX estimates your steps from your WHOOP's motion, calibrated to your phone's step count. It's an estimate, not a step counter. A WHOOP 4.0 doesn't transmit steps.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
                Text("On the days your phone also counted steps, NOX learns how much your motion maps to steps, then applies that to the strap-only days. The more matching days it has, the more it trusts the estimate.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        NoopCard(tint: StrandPalette.metricAmber) {
            VStack(alignment: .leading, spacing: 10) {
                Label("No motion synced yet", systemImage: "antenna.radiowaves.left.and.right.slash")
                    .font(StrandFont.headline)
                    .foregroundStyle(StrandPalette.textPrimary)
                Text("We're not seeing any motion from your strap yet. Steps are estimated from your WHOOP's banked motion history, so your strap needs to sync that history before NOX has anything to count.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
                Text("Open NOX near your strap and let it catch up (a full history sync can take a while on first run). Once a day or two of motion lands, your step estimate and the calibration below will start to fill in.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if profile.stepsCalibrationCoefficient > 0 || profile.stepsManualCoefficient > 0 {
                    ...
                    Text(StepsEstimateEngine.CalibrationStatus
                        ...
                        .font(StrandFont.bodyNumber)
                        .foregroundStyle(StrandPalette.accent)
                    Text("These are the days where your phone also counted steps, so NOX can learn how your motion maps to steps. Or set the coefficient manually below.")
                        .font(StrandFont.footnote)
                        .foregroundStyle(StrandPalette.textTertiary)
                        .fixedSize(horizontal: false, vertical: true)
                }
        ...
        NoopCard {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if comparison.isEmpty {
                    Text("No days yet where both NOX and your phone counted steps. Once your phone logs a few days alongside the strap, they'll appear here so you can see how close the estimate is.")
                        .font(StrandFont.footnote)
                        .foregroundStyle(StrandPalette.textTertiary)
                        .fixedSize(horizontal: false, vertical: true)
                } else {
