...
struct SleepView: View {
        ...
        ScreenScaffold(title: "Sleep", subtitle: "Last night, read in two seconds.",
            ...
            .sheet(item: $addNap) { seed in
                SleepTimeEditor(bedTs: seed.bedTs, wakeTs: seed.wakeTs,
                                title: "Add a nap",
                                blurb: "Pick when the nap started and ended. NOX stages it from your data as its own session, separate from the night's sleep.",
    ...
    private func sleepUndoBanner(_ banner: SleepUndoBanner) -> some View {
        ...
        let message = banner.snapshot.session.userEdited
            ...
            : String(localized: "Sleep deleted. NOX won't detect sleep between \(clockTime(banner.displayStart)) and \(clockTime(banner.windowEnd)) again.")
    ...
    /// whether the score is WHOOP's own imported figure or NOX's on-device estimate. Presentation-only
    ...
    /// Whether the night's sleep-performance score is WHOOP's own imported figure or NOX's
    ...
    /// says "Whoop"; otherwise the night was scored on-device by NOX. Keyed by the night's LOCAL wake-day
    ...
    private func stageBreakdownRow(_ stage: SleepStage, minutes: Double, total: Double) -> some View {
        ...
        HStack(spacing: 10) {
            ...
            // The NOX signature: a segmented PipBar that counts up to the share-of-night fraction,
    ...
    var body: some View {
        ...
        .alert("Delete this sleep session?", isPresented: $confirmingDelete) {
            ...
            Text(suppressesReDetection
                 ? "Removes this recorded sleep and recomputes the day without it. NOX won't re-detect sleep in this window. You can undo for a few seconds after."
